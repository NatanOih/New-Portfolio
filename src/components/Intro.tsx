...
export default function Intro() {
  ...
  if (loading) {
    return (
      <div className="h-[100vh] fixed z-100 top-0 w-full bg-slate-400 flex justify-center items-center">
        loading..
      </div>
    );
  }
  return (
    <section
      ref={ref}
      id="home"
      className=" relative h-full w-full flex flex-col gap-4 p-2 justify-center scroll-mt-28 items-center"
    >
      <div
        className={`font-extrabold relative w-full text-start p-6 select-none overflow-hidden flex flex-col border-[9px] border-black/90 bg-white/10   whitespace-pre uppercase tracking-widest lg:text-[12.2rem] md:text-[6.5rem] sm:text-[5rem]  lg:leading-[9.3rem] md:leading-[6rem] `}
      >
        {introText.map((word, wordIndex) => {
          return <WordTextComponent key={word} word={word} />;
        })}
