...
export default function Header() {
  ...
  return (
    <header className="z-50 relative">
      <motion.div
        className="fixed overflow-hidden top-0 left-1/2 md:h-[4.5rem] w-full rounded-none border opacity-40 bg-slate-200/80 shadow-md shadow-black/[0.13] 
      ...
      <nav className="fixed justify-center flex top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex flex-wrap w-[24rem] items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5 ">
          {links.map((link) => (
            <motion.li
